...
export default async function HomePage({
  ...
  try {
    ...
    [heroData, aboutData, contactData, testimonialsSectionData] = results.slice(
      0,
      4,
    );
  ...
  const heroPrimaryCTA =
    (heroData as any).primaryCTA?.text && (heroData as any).primaryCTA?.href
      ? {
          text: (heroData as any).primaryCTA.text,
          href: (heroData as any).primaryCTA.href,
        }
      : undefined;
  const heroSecondaryCTA =
    (heroData as any).secondaryCTA?.text && (heroData as any).secondaryCTA?.href
      ? {
          text: (heroData as any).secondaryCTA.text,
          href: (heroData as any).secondaryCTA.href,
        }
      : undefined;
  ...
  const aboutCTA =
    (aboutData as any).cta?.text && (aboutData as any).cta?.href
